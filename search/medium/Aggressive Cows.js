...
class Solution {
    ...
    isPossible(arr, distance, k, n) {
        ...
        k -= 1
        for(let i = 1; i < n; i++ ) {
            if (arr[i] - prev >= distance) { // if the distance btw two stalls is more than the distance, we assign a cow and move to next stall
                k--
                prev = arr[i]
            }
            if (k === 0) {
    ...
    aggressiveCows(stalls, k) {
        ...
        while (l <= r) {
            ...
            if (this.isPossible(stalls, mid, k, n)) { // we need to increase the min distance
                l = mid + 1
            } else {
                r = mid - 1
            }
