...
function removeLoop(head) {
    ...
    if (slow == fast) {
        ...
        if (slow != fast) {
            ...
            while (slow.next != fast.next) { // find the node before the start of the loop
                slow = slow.next;
                fast = fast.next;
            }
            ...
            while (fast.next != slow) { // A(1) → B(2) → C(3) → A(1) -> Loops starts at the head, we need to get access to C node to break the loop, 
