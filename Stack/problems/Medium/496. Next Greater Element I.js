...
var nextGreaterElement = function (nums1, nums2) {
  ...
  for (let i = nums2.length - 1; i >= 0; i--) {
    ...
    while (stk.length > 0 && stk[stk.length - 1] <= nums2[i]) {
      // we remove all the smaller elements than the current element, so that the top can contain an element greater than the current element
      stk.pop();
    }
    if (stk.length === 0) {
      map.set(nums2[i], -1);
...
var nextGreaterElement = function (nums1, nums2) {
  ...
  for (const num of nums2) {
    // we iterate from the start, [1, 3, 4, 2]
    while (stack.length && stack[stack.length - 1] < num) {
      // let's say num = 3 and stk = [1], since 1 < 3, the while loop executes, for 1, 3 is the next greater element, so we set in the map
  ...
  while (stack.length) {
    // for the remaining elements in stack, there are no next greater element, so we set it as -1
    map.set(stack.pop(), -1);
  }
