...
class Solution {
  decodedString(s) {
    ...
    for (let i = 0; i < n; i++) {
      cnt = 0;
      if (/[0-9]/.test(s[i])) {
        // if it's a digit, check for the consecutive chars and convert them to an integer and add to the numStack
        ...
        // if we encounter the closing bracket, we need to pop from the charStack till we find an opening bracket
        temp = "";
        ...
        for (let char of res) {
          // push the string again to the stack
          charStack.push(char);
        }
        res = ""; // reset
      } else {
    ...
    while (charStack.length > 0) {
      // pop all chars from the stack and add in reverse
