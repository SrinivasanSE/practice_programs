...
var removeKdigits = function (num, k) {
  ...
  for (let char of num) {
    while (k > 0 && stk.length > 0 && stk[stk.length - 1] > char) {
      // the stack contains a bigger digit than the curr digit, so we pop it
      stk.pop();
      k--;
    }
  ...
  while (k) {
    // this condition will execute for case like [1,2,3,4,5,6], in this case, inside the for loop, pop will not happen, so we remove the last k digits
    stk.pop();
    k--;
  }
  for (let char of stk) {
