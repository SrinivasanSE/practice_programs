...
const findNSE = (arr, n) => {
  ...
  for (let i = n - 1; i >= 0; i--) {
    while (stk.length > 0 && arr[stk[stk.length - 1]] >= arr[i]) {
      // we use >= for NSE and > for PSE, it can be vice versa as well to prevent double counting
      stk.pop();
    }
...
const findPSE = (arr, n) => {
  ...
  for (let i = 0; i < n; i++) {
    // notice the condition here, it's > and not >=. we break ties in NSE and not in PSE, for arr = [3, 3], PSE should be [-1, 0], if we use >=, PSE will be [-1, -1], the subarray [3, 3] will be counted twice.
    // The total should be 9 for [3, 3], subarrays are [3], [3, 3], [3]
    while (stk.length > 0 && arr[stk[stk.length - 1]] > arr[i]) {
      stk.pop();
    }
...
var sumSubarrayMins = function (arr) {
  ...
  for (let i = 0; i <= n; i++) {
    ...
    while (stk.length > 0 && (i === n || arr[i] < arr[stk[stk.length - 1]])) { // should be < and not <=
