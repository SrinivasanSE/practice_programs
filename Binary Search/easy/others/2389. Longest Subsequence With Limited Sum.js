...
var answerQueries = function(nums, queries) {
    ...
    let arr = []
    for (let query of queries) { // For each query, compute how many nums we can take
        let count = 0, sum = 0

        // Keep adding numbers until we exceed the query
        for (let i = 0; i < nums.length; i++) {
            if (sum + nums[i] <= query) { // If adding nums[i] still <= query
...
var answerQueries = function(nums, queries) {
    ...
    for (let i = 0; i < queries.length; i++) {
        // Use binary search:
        // Find how many prefix elements have sum <= query
