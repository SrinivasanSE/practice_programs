...
var maxProfit = function (prices) {
  ...
  const dp = Array.from({ length: n + 1 }, () =>
    new Array(cap * 2 + 1).fill(0)
  ); // The maximum profit you can achieve starting from day i given that you’ve already made tCount buy/sell actions so far.

  for (let i = n - 1; i >= 0; i--) {
    // go from end
    for (let tCount = cap * 2 - 1; tCount >= 0; tCount--) {
      // go from end, we are doing + 1 below, so we can't start from 0
      if (tCount % 2 === 0) {
        // Buy turn
