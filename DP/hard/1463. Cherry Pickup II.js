...
var cherryPickup = function (grid) {
    ...
    const countOfCherries = (i, j1, j2) => { // the robots move together and go to next row together, so i will be same for both
        if (j1 < 0 || j1 > cols - 1 || j2 < 0 || j2 > cols - 1) return -1e9

        if (i === rows - 1) {
            let value = 0
            if (j1 === j2) { // both robots are in the same position, so we consider only one time
                value += grid[i][j1]
            } else {
        ...
        for (let d1 = -1; d1 <= 1; d1++) { // for every move of robot1, robot2 can move in 3 ways, so total 9 ways
...
var cherryPickup = function (grid) {
    ...
    const dp = Array.from({ length: rows }, () => Array.from({ length: cols }, () => Array.from({ length: cols }, () => -1)))
...
var cherryPickup = function (grid) {
    ...
    const dp = Array.from({ length: rows }, () => Array.from({ length: cols }, () => Array.from({ length: cols })))

    for (let j1 = 0; j1 < cols; j1++) { // we assign the last rows value to the dp array based on the logic
            ...
            for (let j2 = 0; j2 < cols; j2++) {
                ...
                for (let d1 = -1; d1 <= 1; d1++) {
                    for (let d2 = -1; d2 <= 1; d2++) {
                        ...
                        if (j1 + d1 >= 0 && j1 + d1 < cols && j2 + d2 >= 0 && j2 + d2 < cols) { // if not out of bounds, consider
