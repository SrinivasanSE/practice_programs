...
var shortestCommonSupersequence = function (str1, str2) {
  ...
  while (i > 0 && j > 0) {
    if (str1[i - 1] === str2[j - 1]) {
      ...
      // if the upper row has the max value, before moving, add the char at that position
  ...
  while (i > 0) {
    // When one string is exhausted (i==0 or j==0), the other string may still have characters left. So we add all remaining ones:
    res = str1[i - 1] + res;
    i--;
  }
