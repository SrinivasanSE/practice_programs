...
class Solution {
  ...
  maximumPoints(arr) {
    ...
    const findMaxPoints = (day, last) => {
      ...
      let points;
      for (let i = 0; i < 3; i++) {
        // 3 cases, selecting 0, 1 or 2
        if (i != last) {
          // ignore the last selected activity and select the remaining two activities
  ...
  maximumPoints(arr) {
    ...
    for (let day = 1; day < n; day++) {
      ...
      for (let last = 0; last < 3; last++) {
        // simulate selecting each activity, if we select 0, then we can't select that and check the remaining two activities
        dp[day][last] = 0;
        for (let j = 0; j < 3; j++) {
          if (j != last) {
