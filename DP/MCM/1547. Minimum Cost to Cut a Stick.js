...
var minCost = function (n, cuts) {
  ...
  for (let i = m - 2; i >= 0; i--) {
    // starts from m - 2, because j loop runs from i + 2 anyway, even if we start from m - 1, the inner loop won't run
    for (let j = i + 2; j < m; j++) {
      // we start from i + 2 because j - i should be greater than 1, if it's 1 that means, no cuts are possible
