...
var coinChange = function (coins, amount) {
  ...
  for (let i = 0; i <= amount; i++) {
    // we converted the base case from recursion, the target can be anything, so we calculate for all the targets
    if (i % coins[0] === 0) {
      dp[0][i] = i / coins[0];
    } else {
      dp[0][i] = 1e9;
...
var coinChange = function (coins, amount) {
  ...
  for (let i = 1; i < n; i++) {
    for (let target = coins[i]; target <= amount; target++) {
      // target can be started from coins[i] instead of 0, prev[0...coins[i] - 1] remains unchanged, even if we start from 0, we will assign the same value again and again since pick will not be there when the target is less than the coin
