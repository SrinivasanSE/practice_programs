...
class Solution {
  cutRod(price) {
    ...
    const findMaxValue = (ind, inches) => {
      ...
      if (ind === 0) {
        // at index 0, the rod length is 1, so if the inches is 5, we can take 5 rods of length 1
...
class Solution {
  cutRod(price) {
    ...
    for (let inch = 1; inch <= n; inch++) {
      // Base case, calc for all inches
    ...
    for (let i = 1; i < n; i++) {
      for (let inch = 0; inch <= n; inch++) {
        ...
        let pick = -1e9;
        const rodLength = i + 1;
        if (rodLength <= inch) pick = price[i] + dp[i][inch - rodLength];
