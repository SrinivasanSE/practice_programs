...
class Solution {
  largestBst(node) {
    ...
    if (isValidBST(node, Number.MIN_SAFE_INTEGER, Number.MAX_SAFE_INTEGER)) { // find the size of the tree if it's valid BST
      return size(node);
    }

    return Math.max(this.largestBst(node.left), this.largestBst(node.right)); // return the max size as we need largest BST
  }
}
...
class Solution {
  largestBst(node) {
    const findLargestBst = (root) => { // we are doing postorder traversal here since we need the value returned by the left and right tree
      if (root == null) {
        // set min as max and max as min value, so the condition executes correctly when we hit the null node
      ...
      if (left.max < root.key && root.key < right.min) {
        // the largest value in the left side should be lesser than the node's val and
        //  smallest value in the right sise should be greater than the node's val to be a valid BST
        return new NodeValue(
          Math.min(left.min, root.key),
          Math.max(root.key, right.max),
