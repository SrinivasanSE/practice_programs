...
class Solution {
  bottomView(root) {
    // from the bottom, whatever node we see, we need to print, if both nodes are in the same level, pick the one which comes later in the level order traversal
    // code here
    const res = [];
    ...
    const dfs = (node, line, level) => {
      ...
      if (!map[line] || map[line][1] <= level) {
        // if the current level is same or greater, pick, for topview, we used >
