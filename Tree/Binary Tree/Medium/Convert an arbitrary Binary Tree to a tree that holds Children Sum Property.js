...
class Solution {
  ...
  changeTree(root) {
    ...
    if (child >= root.val) {
      ...
      if (root.left) {
        // updating left or right with the parent val is fine, no need to update both.
    ...
    /*
        But a leaf node has no children, so there is nothing to sum.

        If we tried to enforce the rule on a leaf, we would get:
