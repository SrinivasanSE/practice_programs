...
var countNodes = function (root) {
  ...
  const leftHeight = getHeight(root, true); // find height, but we are passing only root here, the func will handle it for left and right based on the boolean
  const rightHeight = getHeight(root, false); 
  // for a complete binary tree, if the heights are same, that means it's complete filled from left to right
