...
var buildTree = function(preorder, inorder) { // pre order contains root of the node first
    let preOrderIndex = 0
    ...
    const dfs = (start, end) => {
        ...
        let mid = start
        while (mid <= end && inorder[mid] != rootValue) { // find the root position in the inorder array to find how many elements are in the left and how many are in the right
            mid++
        }
...
var buildTree = function (preorder, inorder) {
    let preOrderIndex = 0, index = 0

    const build = (stop) => { // we use the stop as a boundary for left and right subtree
...
stop for left = root value → left subtree ends at root in inorder
