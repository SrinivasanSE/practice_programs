...
class Solution {
  longestUniqueSubstring(s) {
    ...
    for (let end = 0; end < n; end++) {
      while (set.has(s[end])) {
        // the window should not contain any duplicates, so till the duplicate is removed, we keep removing from the start
        set.delete(s[start]);
        start++;
      }
...
var lengthOfLongestSubstring = function (s) {
  ...
  for (let right = 0; right < n; right++) {
    if (map.has(s[right]) && map.get(s[right]) >= left) {
      // ex: abcc, left = 0, map.get(c) = 2, so we update left to 2 + 1 = 3
...
class Solution {
  longestUniqueSubstring(s) {
    ...
    for (let end = 0; end < n; end++) {
      ...
      ); // if the duplicate is found, then only we move the start
      res = Math.max(res, end - start + 1);
