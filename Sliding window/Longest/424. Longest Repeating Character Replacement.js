...
var characterReplacement = function (s, k) {
  ...
  for (let right = 0; right < s.length; right++) {
    ...
    while (right - left + 1 - maxFreq > k) {
      ...
      maxFreq = 0;
      for (let i = 0; i < 26; i++) {
        // find the maxFreq from the hashmap again since we reduced the frequency
...
var characterReplacement = function (s, k) {
  ...
  for (right = 0; right < n; right++) {
    ...
    if (right - start + 1 - maxFreq > k) {
      // check how many chars needs to be replaced and if it's greater than k or not
