...
// Max Consecutive Ones II is similar, there k is assumed as 1
...
var longestOnes = function (nums, k) {
  ...
  for (right = 0; right < n; right++) {
    ...
    if (k < 0) {
      //if there are more zeros, remove the zero and move the start
      if (nums[start] === 0) {
        // if the start is not zero, the k will remain invalid and we keep moving the start till we find the zero and make k valid
        k += 1;
      }
      start++;
    }
