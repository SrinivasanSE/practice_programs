...
var totalFruit = function (fruits) {
  ...
  for (right = 0; right < n; right++) {
    map.set(fruits[right], (map.get(fruits[right]) || 0) + 1);
    if (map.size > 2) {
      // if can be used instead of while and we move start one at a time. We don't let the window size to increase till it's valid
      count = map.get(fruits[start]) - 1;
      map.set(fruits[start], count);
      if (count === 0) map.delete(fruits[start]);
      start++;
...
var totalFruit = function (fruits) {
  ...
  for (let fruit of fruits) {
    ...
    if (fruit === lastFruit) {
      // if it matches the lastfruit, increment the count, else reset the streak
      lastFruitStreak++;
    } else {
      // we reset the streak here and drop the oldest fruit
      lastFruitStreak = 1;
