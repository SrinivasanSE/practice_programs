...
class Solution {
  subarrayXor(arr, k) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (hashmap.has(rem)) {
        // we know the xor upto the current index and also the subarray's target which is k, if the xor of them is available in the hashmap, that means it's possible to get the subarray with xor k
