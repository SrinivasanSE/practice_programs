...
// MaxHeap approach, MinHeap also can be used, if the order matters, reverse the arr and return it
...
class MaxHeap {
  ...
  heapifyUp() {
    // sort by value if the freq is same
    let index = this.heap.length - 1;
    while (
      index > 0 &&
      (this.heap[index].freq > this.heap[this.parentIndex(index)].freq ||
        (this.heap[index].freq === this.heap[this.parentIndex(index)].freq &&
          this.heap[index].num > this.heap[this.parentIndex(index)].num))
    ) {
  ...
  heapifyDown(index) {
    ...
    if (
      ...
      (this.heap[left].freq > this.heap[largest].freq ||
        (this.heap[left].freq === this.heap[largest].freq &&
          this.heap[left].num > this.heap[largest].num))
    ) {
      largest = left;
    }
    ...
    if (
      ...
      (this.heap[right].freq > this.heap[largest].freq ||
        (this.heap[right].freq === this.heap[largest].freq &&
          this.heap[right].num > this.heap[largest].num))
    ) {
      largest = right;
    }
