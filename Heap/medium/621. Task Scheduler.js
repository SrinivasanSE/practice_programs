...
var leastInterval = function (tasks, n) {
  ...
  while (!heap.isEmpty() || dequeue.length > 0) {
    // O(Tlogk), T tasks will be inserted and extracted from heap
    time++;
    if (!heap.isEmpty()) {
    ...
    if (dequeue.length && dequeue[0][1] === time) {
      // if the current time matches the first one in the queue, that means the time has come for this task to get scheduled
...
var leastInterval = function (tasks, n) {
  ...
  for (let i = 1; i < 26; i++) {
    // by sorting, we try to schedule the most frequents tasks first
...
var leastInterval = function (tasks, n) {
  ...
  for (let i = 0; i < 26; i++) {
    if (freqMap[i] === maxFreq) {
      // we find how many tasks has the same maxFreq
      tasksWithMaxFreq++;
    }
  }
