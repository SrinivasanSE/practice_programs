...
var isNStraightHand = function (hand, groupSize) {
  ...
  while (!heap.isEmpty()) {
    ...
    for (let i = 0; i < groupSize; i++) {
      ...
      if (map.get(temp) === 0) {
        if (temp != heap.getMin()) {
          // if the heap top is not matching, that means we are trying to remove the card which is not in the top
...
var isNStraightHand = function (hand, groupSize) {
  ...
  for (let card of sorted) {
    // even though two for loops are there, the second for loop does not run always and each card only decremented once, so O(N)
    if (map.get(card) === 0) {
      continue;
    }
    freq = map.get(card);
