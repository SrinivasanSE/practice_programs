...
class Solution {
  minJumps(arr) {
    ...
    let jumps = 0,
      ...
      farthest = 0;
    for (let i = 0; i < n - 1; i++) {
      // running upto n - 1 is enough as we don't have to do any jumps from last index
      farthest = Math.max(farthest, i + arr[i]); // The farthest index reachable so far
      if (i === currentEnd) {
        // We’ve used one jump to reach all possible indices up to currentEnd
        if (i === farthest) {
          // if both are equal, that means we can't go from here to the next
          return -1;
        }
        jumps++;
