...
function startStation(gas, cost) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (currGas < 0)
      // if negative,that means can't reach
      return -1;
  }
...
class Solution {
  startStation(gas, cost) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (tank < 0) { // If you fail at station i, then no station between the current start and i can be a valid start.
        // if the fuel becomes negative, then it can't be the starting point, so we reset values
        tank = 0;
        startIndex = i + 1;
      }
    }
    ...
    if (totalGas < 0) {
      // if total fuel is negative at the end, that means it's impossible
      return -1;
    }
