...
const findIndex = (intervals, n, target) => {
  ...
  while (l <= r) {
    ...
    if (target <= intervals[mid][0]) {
      // compare the start with new interval start
      r = mid - 1;
    } else {
      l = mid + 1;
    }
...
const mergeIntervals = (intervals, n) => {
  // same as 56. problem
  let i = 0;
  const res = [];
  while (i < n) {
...
var insert = function (intervals, newInterval) {
  ...
  while (i < n && intervals[i][1] < newInterval[0]) {
    // find the left non overlapping intervals
    res.push(intervals[i]);
    i++;
  }

  while (i < n && intervals[i][0] <= newInterval[1]) {
    // these intervals must be overlapping, compare start with new interval's end and keep updating the new interval
  ...
  while (i < n) {
    // push the nom overlapping intervals in the right side
    res.push(intervals[i]);
    i++;
  }
