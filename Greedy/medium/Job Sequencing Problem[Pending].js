...
// Learn disjoint set and understand the optimal
...
class Solution {
  jobSequencing(deadline, profit) {
    ...
    for (let job of jobs) {
        // pq.size() = number of jobs we have scheduled so far
      if (job.deadline > pq.size()) {
        // we process deadline in ascending oder, if size is 2 and job.deadline is 3,
        // we have already scheduled 2 jobs with deadline less than this which can be completed and this job also can be added
        pq.push(job.profit);
      } // it will come to else if when we have already scheduled the jobs, if the deadline is 4 and we have already scheduled 4 jobs, so we try to find the max profit job
      else if (pq.size() > 0 && pq.top() < job.profit) {
        // if job is already scheduled, check if the profit is higher, if so, replace it
        // for ex: [(1,19), (1,25), (2,100), (2,27), (3,15)] 25 will replace 19 and pq will be [25, 100] after (2, 100) is processed, now when (2, 27) comes,
    ...
    while (pq.size() > 0) {
      // pq holds all the jobs's profit we scheduled
      totalJobs++;
      maxProfit += pq.pop();
    }
